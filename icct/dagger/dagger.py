...
class DAgger:
    ...
    def get_rollout(self, execute_dt=True):
        ...
        while not done:
            ...
            sampled_acts = self.generate_uniform_combinations(n_samples=n_q_samples, act_dim=act_dim).to(device)
            # Use q_networks[0] directly (processed_obs is already extracted features,
            # so we bypass extract_features which fails for dict-obs envs like lane_keeping)
            q1_values = self.oracle.critic.q_networks[0](torch.cat([sampled_obss, sampled_acts], dim=1))
...
class ModifiedDT:
    def __init__(self, env, oracle_model, dt_model, n_rollouts, iterations):
        self.env = env
        self.oracle = oracle_model
        self.dt = dt_model
        self.best_dt = None
