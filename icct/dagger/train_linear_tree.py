...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Training Linear Trees by behavioral cloning from an oracle')
    parser.add_argument('--env_name', help='environment to run on', type=str, default='lunar')
    parser.add_argument('--max_depth', help='the maximum depth of the linear tree', type=int, default=6)
    parser.add_argument('--n_rollouts', help='number of rollouts in the training batch', type=int, default=50)
    parser.add_argument('--iterations', help='unused, kept for argument compatibility with DAgger', type=int, default=80)
    parser.add_argument('--eval_episodes', help='number of episodes to evaluate the trained linear tree', type=int,
                        default=20)
    parser.add_argument('--oracle_load_path', help='the path of loading the oracle model', type=str,
                        default='saved_mlp_models')
    ...
    parser.add_argument('--save', help='the path to save the linear tree model', type=str, default='saved_dt_models')
    parser.add_argument('--load', help='the path to load the linear tree model, skips training', type=str, default=None)
    ...
    if args.load:
        ...
        modified_dt.train(save_dir)
    modified_dt.evaluate(args.eval_episodes)
