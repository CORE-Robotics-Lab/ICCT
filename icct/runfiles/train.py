...
import os
import sys
# Add the project root to the python path so 'icct' can be imported as a package
...
def make_env(env_name, seed):
    ...
    if env_name == 'lunar':
        ...
        env = create_env()  
        name = gym_name
    elif env_name == 'ring_lane_changing_po':
        from flow.utils.registry import make_create_env
        from icct.sumo_envs.accel_ring_multilane_po import ring_accel_lc_po_params
        create_env, gym_name = make_create_env(params=ring_accel_lc_po_params, version=0)
        env = create_env()
        name = gym_name
        ...
        from flow.utils.registry import make_create_env
        from icct.sumo_envs.bottleneck import bottleneck_params
        create_env, gym_name = make_create_env(params=bottleneck_params, version=0)
        env = create_env()
        name = gym_name
    elif env_name == 'merge':
        from flow.utils.registry import make_create_env
        from icct.sumo_envs.merge import merge_params
        create_env, gym_name = make_create_env(params=merge_params, version=0)
        env = create_env()
        name = gym_name
    elif env_name == 'i94':
        from flow.utils.registry import make_create_env
        from icct.sumo_envs.i94 import i94_params
        create_env, gym_name = make_create_env(params=i94_params, version=0)
        env = create_env()
        name = gym_name
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--policy_type', help='mlp, ddt, or oblique_tree', type=str, default='ddt')
    ...
    # sparsity regularization for MLPs
    parser.add_argument('--mlp_sparsity_reg', help='if apply sparsity regularization to MLP actor', action='store_true', default=False)
    parser.add_argument('--mlp_sparsity_type', help='type of sparsity regularization (l1|l2)', type=str, default='l1')
    parser.add_argument('--mlp_sparsity_coeff', help='coefficient for MLP sparsity regularization', type=float, default=1e-4)
    parser.add_argument('--mlp_sparsity_include_bias', help='include bias terms in sparsity regularization', action='store_true', default=False)
    ...
    # If SUMO_HOME is set, add its bin to PATH (needed for some environments)
    ...
    if args.env_name == 'lane_keeping':
        eval_env = gym.wrappers.TimeLimit(gym.make(env_n).unwrapped, max_episode_steps=500)
    else:
    ...
    if args.policy_type == 'ddt':
        if not args.submodels and not args.hard_node:
            ...
            if args.sparse_submodel_type == 1:
                method = 'm2_l1'
            elif args.sparse_submodel_type == 2:
                method = f'm2_e{args.num_sub_features}'
            else:
                method = 'm2'
    ...
    elif args.policy_type == 'oblique_tree':
        method = 'oblique_tree'
    ...
    if args.policy_type == 'ddt' or args.policy_type == 'oblique_tree':
        ddt_kwargs = {
            ...
            'use_oblique': args.policy_type == 'oblique_tree'
        }
        policy_kwargs = {
        ...
        if args.mlp_size == 'small':
            if args.env_name == 'cart':
                ...
                pi_size = [3, 3]
            elif args.env_name == 'ring_lane_changing_po':
                pi_size = [3]
            elif args.env_name == 'figure8':
                pi_size = [3, 3]
            else:
                pi_size = [2]
        elif args.mlp_size == 'medium':
    ...
    if args.alg_type == 'sac':
        model = SAC(policy_name, env,
                    ...
                    device=args.device,
                    seed=args.seed,
                    mlp_sparsity_reg=args.mlp_sparsity_reg if args.policy_type == 'mlp' else False,
                    mlp_sparsity_type=args.mlp_sparsity_type,
                    mlp_sparsity_coeff=args.mlp_sparsity_coeff,
                    mlp_sparsity_include_bias=args.mlp_sparsity_include_bias)
    else:
