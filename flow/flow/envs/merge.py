...
class MergePOEnv(Env):
    ...
    * num_rl: maximum number of controllable vehicles in the network
    ...
    def observation_space(self):
        """See class definition."""
        return Box(low=0, high=1, shape=(16 * self.num_rl, ), dtype=np.float32)

    def _apply_rl_actions(self, rl_actions):
        acceleration = rl_actions[::2][0]
        direction = rl_actions[1::2][0]
        """See class definition."""
    ...
    def get_state(self, rl_id=None, **kwargs):
        ...
        observation = [0 for _ in range(16 * self.num_rl)]
        for i, rl_id in enumerate(self.rl_veh):
            ...
            lane = self.k.vehicle.get_lane(rl_id) / max_lanes


            observation[16 * i + 0] = this_speed / max_speed
            observation[16 * i + 1] = (lead_speed - this_speed) / max_speed
            # observation[5 * i + 2] = lead_head / max_length
            observation[16 * i + 2] = (this_speed - follow_speed) / max_speed
            # observation[5 * i + 4] = follow_head / max_length
            observation[16 * i + 3] = lane
