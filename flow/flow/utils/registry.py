...
def make_create_env(params, version=0, render=None):
    ...
    # gym versions differ: older versions expose registry.all(), newer
    # versions expose registry as a dict mapping id -> EnvSpec.
    registry = gym.envs.registry
    if hasattr(registry, 'all'):
        all_envs = registry.all()
        env_ids = [env_spec.id for env_spec in all_envs]
    else:
        # registry is a dict-like mapping from env id -> EnvSpec
